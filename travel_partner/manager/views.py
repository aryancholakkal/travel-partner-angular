...
def view_dashboard_manager(request):
    ...
    try:
        leave_requests = TicketDetails.objects.filter(manager_id=request.user.manager.id, employee_cancel_status = False)
...
def approve_ticket_manager(request, ticket_id):
    ...
    if request.method == 'PUT':
        ...
        ticket.additional_request_manager = feedback.get('feedback')
        ticket.save()
...
def filter_dash(request):
    ...
    if request.method == "GET":
        ...
        serializer = TicketDetail_for_manager(records, many=True)
        return Response(serializer.data, status=HTTP_200_OK) 
