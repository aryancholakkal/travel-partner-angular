...
def view_dashboard_admin(request):
    ...
    """
    try:
        leave_requests = TicketDetails.objects.filter(admin_ticket_status = 'Open')
...
def filter_dash(request):
    ...
    if request.method == "GET":
        ...
        if start_date:
            records = records.filter(end_date__gte=start_date)
        if end_date:
        ...
        serializer = TicketDetail_for_admin(records, many=True)
        return Response(serializer.data, status=HTTP_200_OK) 
...
@api_view(['PUT'])
@permission_classes([IsAdmin])
def request_edit(request, ticket_id):
    ...
    if request.method == 'PUT':
        ...
        ticket.manager_ticket_status = "Request Edit"
        ticket.additional_request_admin = feedback.get('feedback')
        ticket.save()
        send_mail(
